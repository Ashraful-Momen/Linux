...
# Set user + group together :
setfacl -m u:username:rwx,u:username:r--,g:groupname:rwx filename
...
### Activity Logging
```bash
# Start logging terminal session to a file - (whaterver you do in terminal, those activities write in a file as log.)
script filename.txt
script -f /path/to/logfile.txt  # Force write

# View real-time file access: show the file logs real time >>> tail -n 20 -f file_name  <---------- -n = number of line you want to see. 
